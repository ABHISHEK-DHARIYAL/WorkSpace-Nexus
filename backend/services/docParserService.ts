...
const getPDFParse = () => {
  const PDFAny: any = PDFParseModule;

  if (PDFAny.PDFParse) return PDFAny.PDFParse;
  if (PDFAny.default?.PDFParse) return PDFAny.default.PDFParse;
  if (typeof PDFAny.default === "function") return PDFAny.default;
  if (typeof PDFAny === "function") return PDFAny;
...
export class DocParserService {
  ...
  static async parsePdf(
    ...
    if (!PDFParseClass) {
      throw new Error(
        "PDF parser not properly initialized. This may be an environment issue."
      );
    }
    ...
    try {
      ...
      await ListingService.update(listing.id, {
        ...
        index: index.map((item: any) => ({
          ...item,
          pageId: createdPages.find((p) => p.pageNumber === item.pageNumber)
            ?.id,
        })),
      });
  ...
  static async parseDocx(
    ...
    const elementRegex =
      /<(h1|h2|h3|h4|h5|p|table|ul|ol)[^>]*>([\s\S]*?)<\/\1>/gi;
