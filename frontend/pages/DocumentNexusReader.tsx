...
const DocumentNexusReader: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProjectDetails = async () => {
      try {
        setLoading(true);
        const res = await publicService.getProjectById(id!);
        const data = res.data;
        if (!data) {
        ...
        console.error("Failed to load Document Nexus project:", err);
  ...
  return (
    <div className={`min-h-screen flex flex-col pt-16 transition-colors duration-300 ${themeClasses.backdrop}`}>
      ...
      <header id="nexus-reader-top-toolbar" className={`min-h-[4rem] h-auto py-3 lg:py-0 flex flex-col lg:flex-row lg:items-center justify-between px-4 sm:px-8 z-30 gap-4 ${themeClasses.headerToolbar}`}>
        <div className="flex items-center gap-3">
          <Link
            ...
            title="Back to Public Content Explorer"
          >
            <ArrowLeft size={14} />
          </Link>
          ...
          {isSidebarCollapsed && (
            ...
            >
              <LayoutList size={14} />
              <span className="hidden sm:inline">Expand Sidebar Outline</span>
              <span className="inline sm:hidden">Outline</span>
            </button>
          )}
          <div>
            <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-950/20 dark:text-emerald-400 dark:border-emerald-900/30">
              🌍 Public Document Nexus Publication
            </span>
            <div className="flex items-center gap-2.5">
              ...
              <h1 className="text-base font-black truncate max-w-md">
                {project ? project.title : 'Published Compilation'}
              </h1>
            </div>
          </div>
        </div>
                  ...
                  <button
                    ...
                    className={`p-2 rounded-xl border transition-all ${
                      isFullScreen
                        ...
                        : 'bg-slate-50 dark:bg-[#1f242e] border-slate-200 text-slate-500 hover:bg-slate-100'
                    }`}
                ...
                pagesToRender.map((page, pIdx) => (
                  ...
                  >
                    <div className="p-8">
                      ...
                      <div className="border-b pb-4 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-2 border-slate-100 dark:border-slate-800">
                        <div>
                          ...
                          {page.createdAt && (
                            <p className="text-[9px] font-mono text-slate-400 flex items-center gap-1 mt-0.5 select-none">
                    ...
                    <div className={`p-4 border-t flex justify-between items-center text-[10px] font-black uppercase tracking-wider transition-all duration-300 ${themeClasses.cardFooter}`}>
                      ...
                      <span className="text-slate-400 dark:text-slate-500 font-mono">Doc-Nexus Engine Block</span>
                    </div>
                  </motion.div>
                ))
              )}
...
export default DocumentNexusReader;
